...
var FB,rstar,r, rk,  w, b,  y, varpi, s, inv, invG, c, cw,  cr,  tauw, N, stoyw, PiF,Tw,  hw, Dr, Dw,  ep, varsig, zetar, zetay, gw, g, gE, iy, gpc, zz, far, faw, dr, dw,  mu, k, u, del, delprime, gM,   v, j,  lam, gA, za, PiA, PiRD,  fa, n , gn, gy, gamma, OMEGAY, fert, psi, tauwE, tauwA, ay, tpe, Pe, he,en,ey,er,shockn,shocky,shockr,shareW, shareR, shockR;
varexo delall; 
...
PiF = (rk + 1)*(k(-1)/g) + (b(-1)*r(-1)+FB(-1)*rstar(-1))/g - fa(-1)*r(-1)/g - k - b -FB + fa + (1-SHINNOVW)*(PiA + PiRD); %financial profits

%r= rstar;
%not working
%r(1)=rstar(1);
%working
%r(1)/rstar(1)-1=.99*(r/rstar-1);
%good working!
r(1)-rstar(1) = .99*(r - rstar) ;
%;
%r-rstar = .99*(r(-1) - rstar(-1)) ;
%r/rstar-1=.99*(r(-1)/rstar(-1)-1);
%not working

%r(1)/r-1=0.9*(rstar(1)/rstar-1);
%not working
%r(1)-R_SS=0.9*(rstar(1)-R_SS);
%woring
%+0.4*(rstar-rstar(-1))+0.5*(rstar(-1)-rstar(-2));
