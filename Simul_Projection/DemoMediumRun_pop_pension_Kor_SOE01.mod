...
dr = PiF*far(-1)/(fa(-1)+Fb(-1)); %financial dividends to retirees
dw = PiF*faw(-1)/(fa(-1)+Fb(-1)) + SHINNOVW*(PiA + PiRD); %financial dividends to workers
b = s +  varpi*(1 -1/za(-1)) ; %total bank loans
PiF = (rk + 1)*(k(-1)/g) + b(-1)*r(-1)/g + Hbstar(-1)*rstar(-1)/g -r(-1)/g*(fa(-1) + Fb(-1)) - k - b - Hbstar + Fb + fa + (1-SHINNOVW)*(PiA + PiRD); %financial profits
Hbstar = rstar-r;
%r(+1)-R_SS=rstar-R_SS;
%r(+1)=rstar(+1);
%ugly result
%r(+1) = rstar;
%strange working
%r(+1)-rstar(+1) = 0.95*(r(-1)-rstar(-1));
%wierd result
%r-rstar = 0.95*(r(-1)-rstar(-1));
%not working
%r(+1)-rstar(+1) = 0.95*(r-rstar);
%working
%r(+1)/rstar(+1)-1 = 0.9*(r/rstar-1);
%AR(1) working
%Fb = (r/rstar-1);
%working but... bad result
%market clearing conditions
k = (1-del)*(k(-1)/g) + inv; %evolution of investment 
y = (1-GAMMAI/(VARNU*mu)) - N*BMEGA*psi; %total output
cu = Hbstar - Fb;
cu = y -( c + inv + s + varpi*(1-1/za(-1)) + tauwE + he);
%current account
%y = c + inv + s + varpi*(1-1/za(-1)) + tauwE + he; %output identity
c = cr + cw; %total consumption

%total bond
fa = k + b + cu ; %asset markets clearing (financial assets of ertirees and workers is equalized to future loans and capital stock 
