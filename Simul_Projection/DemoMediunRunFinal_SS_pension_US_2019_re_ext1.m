...
function flag = DemoMediunRunFinal_SS_ext1
x0_R = 1.1;
...
omegar = 1 -1/(45);%1 -1/(45); %set such that individuals work for 45 years 
omegay = 1 - 1/20;%1 - 1/15; % set such that young take 15 years (school) to become worker
gamma = 1 - 1/5;%1 - 1/10; %set such that retirees live for 10 years on average
fert_SS = n_SS - omegay;
deltaHE = 0.12;%0;%0.12;
np = 25;
etar = 0.4;%0;%0.4;
...
aveage = 45; % 원래 40
lamy = 1/(aveage-20+1);%1/3; %1/3; 1; % 20% of the young that become workers in the past 15 years contribute to innovation effort
...
load RelatAgeDist_Jones_1Y_extend.txt
%RelatAgeDist_Jones_1Y = RelatAgeDist_Jones_1Y/RelatAgeDist_Jones_1Y(1,1);
Jon = RelatAgeDist_Jones_1Y_extend';
Jon1 = Jon;
...
k_SS = g_SS*alpha*(1-gammaI)/(mu_SS*(rk_SS+delta)); % Put a plus tauK after rk_SS to include capital tax
...
he_SS = deltaHE*(zetar_SS/(1+zetar_SS+zetay_SS));
c_SS = y_SS - (inv_SS + s_SS + varpi_SS - varpi_SS/za_SS) - tauwE_SS - he_SS;
b_SS = s_SS + varpi_SS - varpi_SS/za_SS;
fa_SS = k_SS+b_SS;

PiF_SS = (rk_SS +1)*k_SS/g_SS + (R_SS/g_SS)*(b_SS - fa_SS) -k_SS - b_SS + fa_SS + (1-shinnovw)*PiA_SS; 

ay_SS = (1/np)*w_SS*(1-omegar)/(1-(1-1/np)*gw_SS/g_SS);
tpe_SS = etar*ay_SS/(1-gamma*gE_SS/g_SS);
tauwA_SS = tpe_SS + he_SS;
tauw_SS = tauwE_SS + tauwA_SS;
hw_SS = w_SS/(1 - omegar*g_SS/(R_SS*gw_SS*Z_SS));
far_SS = (fa_SS*(1-omegar)/omegar +(1-Epvarsig_SS*(1-gamma*g_SS/(R_SS*gN_SS))^(-1))*tpe_SS)/(1 + (Epvarsig_SS-1)*R_SS/g_SS + (Epvarsig_SS*(1-gamma*g_SS/(R_SS*gN_SS))^(-1)-1)*(PiF_SS/fa_SS)+ (1-omegar)/omegar);
dr_SS = PiF_SS*far_SS/fa_SS;
Dr_SS = dr_SS*(1-gamma*g_SS/(R_SS*gw_SS))^(-1);
Pe_SS = tpe_SS*(1-gamma*g_SS/(R_SS*gw_SS))^(-1);
faw_SS = fa_SS - far_SS;
dw_SS = PiF_SS*faw_SS/fa_SS + shinnovw*PiA_SS;
Tw_SS = tauw_SS/(1 - omegar*g_SS/(R_SS*gN_SS*Z_SS));
Dw_SS = (dw_SS + (1/zetar_SS)*((Dr_SS+Pe_SS)/(R_SS*Z_SS))*(g_SS/gw_SS)*(1-omegar)*(Ep_SS)^((rhoU - 1)/rhoU))*((1-omegar*g_SS/(R_SS*Z_SS*gw_SS))^(-1));


cr_SS = Epvarsig_SS*(far_SS*R_SS/g_SS+Dr_SS + Pe_SS);
...
test4 = (w_SS-c_SS+PiF_SS - tauw_SS+tpe_SS)/(1-R_SS/g_SS)-fa_SS;
...
test6 =dw_SS + (omegar/(R_SS*Z_SS))*(g_SS/n_SS)*Dw_SS + ((1-omegar)*Ep_SS^((rhoU-1)/rhoU)/(R_SS*Z_SS))*(g_SS/(zetar_SS*n_SS))*(Dr_SS+Pe_SS) - Dw_SS;
test7 = -far_SS + far_SS*R_SS/g_SS + dr_SS + tpe_SS - cr_SS + (1-omegar)*(faw_SS*R_SS/g_SS +w_SS+dw_SS-cw_SS - tauw_SS); 
...
save paramFinal_ext1 gN_SS rhoyw bbeta ratiodel delta alpha rho varnu gammaI phi Bmega chi ratiodel elasmu phi rhoU gamma omegar omegay delprime el_SS chiE rhoE lamy yinnovsh etar deltaHE np shinnovw
save vecSSFinal_ext1 w_SS k_SS g_SS  gw_SS gE_SS iy_SS tauw_SS Tw_SS ga_SS c_SS y_SS mu_SS rk_SS  U_SS  R_SS lambda_SS el_SS  s_SS za_SS  v_SS  j_SS  varpi_SS b_SS fa_SS PiF_SS Z_SS Epvarsig_SS Ep_SS varsig_SS zetar_SS zetay_SS faw_SS far_SS cr_SS cw_SS hw_SS gm_SS gamma_SS n_SS Dr_SS Dw_SS dr_SS dw_SS PiA_SS inv_SS psi_SS stoyw_SS fert_SS replac_SS tauwA_SS tauwE_SS ay_SS tpe_SS Pe_SS he_SS
