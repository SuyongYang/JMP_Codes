...
%Shinnovw??
PiF = (rk + 1)*(k(-1)/g) + b(-1)*r(-1)/g + ca(-1)*rstar(-1)/g - fa(-1)*r(-1)/g - k - b - ca + fa + (1-SHINNOVW)*(PiA + PiRD) - abs(fee(-1)*ca(-1))/g; %financial profits
%Modified with international fin. cost
...
%y = c + inv + s + varpi*(1-1/za(-1)) - sign(fee)*abs(ca) + tauw; %output identity
%modified
...
%fa = k + b- sign(fee)*abs(ca);
%modified
far = far(-1)*r(-1)/g + dr - cr + (1-OMEGAR)*(faw(-1)*r(-1)/g +w+dw-cw - tauw) ;
fa  = far+faw; %financial assets

%r(1)-rstar(1)=R_SS-Rstar_SS + delall*((r-rstar)-(R_SS-Rstar_SS))+ delall(-1)*((r-rstar)-(R_SS-Rstar_SS));
%not working
%1+rk(1)=R_SS + delall*(rstar-R_SS);
% not working
%r(1)/rstar(1) - (R_SS/Rstar_SS) = 0.9*((r/rstar) - (R_SS/Rstar_SS));
% not
%r(1)-rstar(1) = (R_SS-Rstar_SS)*((r-rstar)/(R_SS-Rstar_SS))^2;
% not
%exp((r(1)-rstar(1))*100)=exp((r-rstar)*100);
% good
%tan((r(1)-rstar(1))*10)=tan((r-rstar)*10);
% good
ln((r(1)-rstar(1)))=ln((r-rstar));
%r-rstar - (R_SS-Rstar_SS) = 0.9*(r(-1)-rstar(-1) - (R_SS-Rstar_SS))  + delall*0.5;
%not working
%r-R_SS = rstar - Rstar_SS;
% not


fee=r-rstar;

%path = (sign(fee*ca)*(fee*ca))^2;
path = (ca);
pathA= (fee*ca)/(g);
