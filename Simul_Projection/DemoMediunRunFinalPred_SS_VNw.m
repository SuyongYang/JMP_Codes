...
% This code generates the steady state for the the model with working
% population in the congestion term.
function flag = DemoMediunRunFinalPred_SS_VNw
x0_R = 1.1;
...
TotalPopUS2060 =   170.802; %workingage 210 (15-64)
...
omegarr = 1 -1/(40);%1 -1/(45); %set such that individuals work for 45 years 
...
Z_SS = omegarr + (1-omegarr)*(Ep_SS)^((rhoU - 1)/rhoU);
zetay_SS = (n-omegarr)/(1-omegay);
zetar_SS = (1-omegarr)/(gN_SS-gamma);
shareW_SS = 1/(1+zetay_SS+zetar_SS);
shareR_SS = zetar_SS/(1+zetay_SS+zetar_SS);

replac_SS = 40*0.45*fert_SS*zetay_SS;

stoyw_SS = stoyw_SS*(1+zetar_SS + zetay_SS); %adjust stoyw since now it must be share of 2060
aveage = 40;
lamy = 1/((aveage-20+1)*omegarr);
yinnovsh = (1-(1-lamy)*omegarr/gN_SS)*stoyw_SS/((1-omegay)*zetay_SS);
newyoung = yinnovsh*(1-omegay)*zetay_SS;
...
popinnov = 0;
for iii = 1:1:43
    shareinnov(iii) =  newyoung*((1-lamy)*omegarr/gN_SS)^(iii-1);
...
iy_SS = ((gw_SS^(-1))*(g_SS*2*bbeta*(1-omegay)*zetay_SS)*((gE_SS*gw_SS - omegarr)/((1-omegay)*zetay_SS) - rhoE));
...
hw_SS = w_SS/(1 - omegarr*g_SS/(R_SS*gw_SS*Z_SS));
far_SS = (fa_SS*(1-omegarr)/omegarr)/(1 + (Epvarsig_SS-1)*R_SS/g_SS + (Epvarsig_SS*(1-gamma*g_SS/(R_SS*gN_SS))^(-1)-1)*PiF_SS/fa_SS+ (1-omegarr)/omegarr);
dr_SS = PiF_SS*far_SS/fa_SS;
Dr_SS = dr_SS*(1-gamma*g_SS/(R_SS*gw_SS))^(-1);
faw_SS = fa_SS - far_SS;
dw_SS = PiF_SS*faw_SS/fa_SS + shinnovw*PiA_SS;;
Tw_SS = tauw_SS/(1 - omegarr*g_SS/(R_SS*gN_SS*Z_SS));
Dw_SS = (dw_SS + (1/zetar_SS)*(Dr_SS/(R_SS*Z_SS))*(g_SS/gw_SS)*(1-omegarr)*(Ep_SS)^((rhoU - 1)/rhoU))*((1-omegarr*g_SS/(R_SS*Z_SS*gw_SS))^(-1));
...
test3 = (omegarr)*(faw_SS*R_SS/g_SS+w_SS-cw_SS+ dw_SS - tauw_SS)-faw_SS;
...
test6 =dw_SS + (omegarr/(R_SS*Z_SS))*(g_SS/n_SS)*Dw_SS + ((1-omegarr)*Ep_SS^((rhoU-1)/rhoU)/(R_SS*Z_SS))*(g_SS/(zetar_SS*n_SS))*Dr_SS - Dw_SS;

par = [bbeta ratiodel delta alpha rho varnu gammaI phi Bmega chi ratiodel elasmu phi rhoU gamma omegarr omegay delprime el_SS chiE rhoE];
...
save paramFinalPred gN_SS rhoyw bbeta ratiodel delta alpha rho varnu gammaI phi Bmega chi ratiodel elasmu phi rhoU gamma omegarr omegay delprime el_SS chiE rhoE lamy yinnovsh shinnovw
save vecSSFinalPred shareW_SS shareR_SS w_SS k_SS g_SS  gw_SS gE_SS iy_SS tauw_SS Tw_SS ga_SS c_SS y_SS mu_SS rk_SS  U_SS  R_SS lambda_SS el_SS  s_SS za_SS  v_SS  j_SS  varpi_SS b_SS fa_SS PiF_SS Z_SS Epvarsig_SS Ep_SS varsig_SS zetar_SS zetay_SS faw_SS far_SS cr_SS cw_SS hw_SS gm_SS gamma_SS n_SS Dr_SS Dw_SS dr_SS dw_SS PiA_SS inv_SS psi_SS stoyw_SS fert_SS replac_SS



function y = Findvarsig(x)
global  bbeta R_SS omegarr rhoU Epvarsig_SS
y = 1 - bbeta^(1/(1-rhoU))*(R_SS*(omegarr + (1-omegarr)*(Epvarsig_SS/x)^((rhoU - 1)/rhoU)))^(rhoU/(1-rhoU)) -x;
