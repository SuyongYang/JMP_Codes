...
totpop=readtable('ByAgeProj2019.0.975.xlsx');
totpop=table2array(totpop);
Countries = fileread('listcountries.txt');

%get weights
mh = size(totpop,1);
%mp = size(ProjecPopData,1);
Popweights = zeros(mh,3);
for i = 1:mh
    sumpop = sum(totpop(i,1:end));
    Popweights(i,1) = sum(totpop(i,1:4))/sumpop;
    Popweights(i,2) = sum(totpop(i,5:12))/sumpop;
    Popweights(i,3) = sum(totpop(i,13:end))/sumpop;
end

i = flag;
totpophist = sum(totpop(:,1:end),2);
gpop = diff(log(totpophist)); % 2000~2100 -> 2001-2100(growth)
difgpop = gpop - gpop(1); % 2001 1.01% !
gN_SS = 1 + 0.01;
%testgpop = gN_SS-1+difgpop;
dDW_3g = diff(Popweights);
for iii = 1:30
                dd_old = 1+gpop(5+iii) ;
                dd = gN_SS + difgpop(5+iii); %2006 yr grwth shock starts
