...
stoyw = YINNOVSH*(1-OMEGAY)*zetay(-2)/(1+zetay(-2)+zetar(-2))/gn(-1) + ((1-LAMY)*OMEGAR/gn(-1))*stoyw(-1); % intensity to transform invented goods into adopted goods
%modify Ny(t) -> Ny(t-1)
%original:stoyw = YINNOVSH*(1-OMEGAY)*zetay(-1)/(1+zetay(-1)+zetar(-1)) + ((1-LAMY)*OMEGAR/gn(-1))*stoyw(-1); % intensity to transform invented goods into adopted goods
...
% FB>0 forever
PiF = (rk + 1)*(k(-1)/g) + (b(-1)*r(-1)+FB(-1)*r(-1))/g - fa(-1)*r(-1)/g - k - b -FB + fa + (1-SHINNOVW)*(PiA + PiRD); %financial profits
% FB<0 forever
...
%PiF_star = (rk_star + 1)*(k_star(-1)/g_star) + (b_star(-1)*r_star(-1)-gdpratio*FB(-1)*g(-1)/g_star(-1)*r_star(-1))/g_star - fa_star(-1)*r_star(-1)/g_star - k_star - b_star + gdpratio*FB*g/g_star + fa_star + (1-SHINNOVW_star)*(PiA_star + PiRD_star); %financial profits
% FB>0 forever
