...
function flag = DemoMediunRunFinal_SS
x0_R = 1.1;
...
hold on
yyaxis left
h1 = plot(z, shareinnov, '-o', 'LineWidth', 2);
axis([20 62 0 0.002])
ylabel(texlabel('Share of Workers in R&D - S_{gamma, j}'))
yyaxis right
z = 20:1:62;
h2 = plot(z, Jon(1:end), '-.', 'LineWidth', 2);
h = legend([h1, h2], {texlabel('Share of Workers'), texlabel('Relative Productivity - Jones(2010)')});
set(h,'Position', [0.65 0.93 0.05 0.05], 'Orientation', 'horizontal','Box', 'off');
...
save paramFinal gN_SS rhoyw bbeta ratiodel delta alpha rho varnu gammaI phi Bmega chi ratiodel elasmu phi rhoU gamma omegar omegay delprime el_SS chiE rhoE lamy yinnovsh shinnovw
