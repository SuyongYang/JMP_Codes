...
% This code generates the steady state and new rho_yw when age profile of
% innovation changes

function flag = DemoMediunRunFinal_SS_ext3
x0_R = 1.1;
finalR_SS = fsolve(@RunDemoMediumRunFinal_SS,x0_R);
R_SS = finalR_SS;
[test, ~,vecSS, vectest] = RunDemoMediumRunFinal_SS(finalR_SS);
flag = 'done';
...
stoyw_SS = TotalResearchersUS/TotalPopUS; %Share of Workers in R&D in the US.shinnovw = 0; %10% of profits go to workers who are in innovation
...
yy = 5;
Jon2 = zeros(1,43+yy);
Jon2(1,17:end) = Jon(17-yy:end);
Jon2(1,1) = Jon(1,1);
Jon2(1,2) = Jon(1,2)*0.64;
for i=3:16
     Jon2(1,i) = ((i-2)/(17-2))*(Jon2(1,17) - Jon2(1,2))+Jon2(1,2);
end 
%given lamy and stoyw_SS find rho_yw
rhs_obj = 0;
popinnov = 0;
...
rhs_obj = rhs_obj +  sum(shareinnov.^(Jon2(1:end)));
rhs_obj = rhs_obj + (stoyw_SS-popinnov);
rhoyw_end = log(rhs_obj)/log(stoyw_SS)-1;
...
yyaxis right
z = 20:1:62;
h1 = plot(z, Jon(1:end), '-', 'LineWidth', 2);
z = 20:1:62+yy;
h2 =plot(z, Jon2(1:end), ':', 'LineWidth', 2);
h = legend([h1, h2], {texlabel('Age Distribution - Jones(2010)'), texlabel('Shifted Age Distribution')})
axis([20 62+yy 1 max(Jon)+1])
ylabel(texlabel('Relative Productivity - Phi_j'))
xlabel('Age')
;
set(h,'Position', [0.6 0.93 0.05 0.05], 'Orientation', 'horizontal','Box', 'off');
...
pers2 = 0.99;
difrhoyw = (rhoyw_end - rhoyw)*(1-pers2);

save paramFinal_ext3 gN_SS rhoyw bbeta ratiodel delta alpha rho varnu gammaI phi Bmega chi ratiodel elasmu phi rhoU gamma omegar omegay delprime el_SS chiE rhoE lamy yinnovsh difrhoyw shinnovw pers2
save vecSSFinal_ext3 w_SS k_SS g_SS  gw_SS gE_SS iy_SS tauw_SS Tw_SS ga_SS c_SS y_SS mu_SS rk_SS  U_SS  R_SS lambda_SS el_SS  s_SS za_SS  v_SS  j_SS  varpi_SS b_SS fa_SS PiF_SS Z_SS Epvarsig_SS Ep_SS varsig_SS zetar_SS zetay_SS faw_SS far_SS cr_SS cw_SS hw_SS gm_SS gamma_SS n_SS Dr_SS Dw_SS dr_SS dw_SS PiA_SS inv_SS psi_SS stoyw_SS fert_SS replac_SS
